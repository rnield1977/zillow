...
function(address, citystatezip, zillowId = getOption("ZillowId", stop("need zillow id")), ...)  
{
  ...
  code <- checkStatus(doc)
  
  
  if (code == "508"){
    return(data.frame(amount = 0,
               low = 0,
               high = 0,
               valueChange30Day = 0,
               rentamount = 0,
  ...
  data.frame(amount = as.numeric(xmlValue(est[["amount"]])),
             ...
             rentamount = as.numeric(xmlValue(rent[['amount']])),
             renthigh   = as.numeric(xmlValue(rent[['valuationRange']][['high']])),
             rentlow   = as.numeric(xmlValue(rent[['valuationRange']][['low']])),
             row.names = zpid )
}
